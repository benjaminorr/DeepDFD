...
def autocorrelation1d_symmetric(h):
    ...
    Fhsq = complex.abs2(complex.rfft(h, 1))
    a = complex.irfft(torch.stack([Fhsq, torch.zeros_like(Fhsq)], dim=-1), 1, signal_sizes=(h.shape[-1],))
    return a / a.max()
...
def edgetaper3d(img, psf):
    ...
    blurred_img = complex.irfft(
        complex.multiply(complex.rfft(img, 2), complex.rfft(psf, 2)), 2, signal_sizes=img.shape[-2:]
    )
